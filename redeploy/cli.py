...
def _run_apply(console, migration, dry_run, output, ssh_key: str = "",
    ...
    if no_state:
        ...
        state_path = Path("/dev/null")  # placeholder; executor will use it but never saved
    executor = Executor(
        migration,
        dry_run=dry_run,
...
@click.option("--host", default=None,
              help="Override target host (default: spec.target.host)")
@click.option("--ssh-key", default=None, type=click.Path(),
              help="Path to SSH private key")
@click.option("--format", "output_fmt", default="rich",
              type=click.Choice(["rich", "yaml", "json"]),
              help="Report format")
@click.option("--exit-on-fail", is_flag=True,
...
def diagnose(ctx, spec, host, ssh_key, output_fmt, exit_on_fail):
    ...
    if output_fmt == "yaml":
        ...
        icon = {"pass": "[green]✓[/green]", "fail": "[red]✗[/red]",
                "warn": "[yellow]![/yellow]", "skip": "[dim]·[/dim]"}
        for c in report.checks:
            t.add_row(icon.get(c.status, c.status),
                      c.category, c.name,
                      c.detail[:60], c.source_step)
        console.print(t)
